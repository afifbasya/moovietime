<template>
    <section class="pt-14 pb-12">
        <Carousel :items-to-show="2.5" :wrap-around="true" :autoplay="3000">
            <Slide v-for="(item, index) in now_playing.results.filter((item, index) => index < maxShows)" :key="index">
                <div class="carousel__item">
                    <CommonCarouselCard
                        :url="'/movies/' + item.id"
                        :rate="item.vote_average?.toFixed(1)"
                        :genres="item?.genre_ids?.map(genre => dataCategories?.genres?.find(category => genre === category.id)?.name)?.join(', ')"
...
const { now_playing } = useListMovie();
